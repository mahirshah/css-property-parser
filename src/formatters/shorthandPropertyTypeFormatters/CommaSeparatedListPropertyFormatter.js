...
module.exports = class CommaSeparatedListPropertyFormatter {
   ...
   *
   * @param {string} propertyName - the css property name. For example, 'animation'.
   * @param {Object} node - the root node of the nearley parser tree
   * @param {string} value - the property value string. For example, "move-left 1s 1s".
   * @returns {Object} - a mapping between longhand property names to their longhand values.
   */
  static format(propertyName, node, value) {
    return Object
      .entries(ShorthandPropertyTypeFormatterUtils.getPropertyLocationMappingCommaSeparatedList(propertyName, node))
